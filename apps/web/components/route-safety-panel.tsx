...
export function RouteSafetyPanel({ routes, recomputeAction }: RouteSafetyPanelProps) {
  return (
    <article className="panel">
      <PanelGate
        requiredPanel="routes"
        panelName="Corridor Intelligence"
        fallbackDescription="Scout tier unlocks live corridor intel."
      >
        <p className="panel-label">Corridor Safety · EVE Frontier</p>
        <h2>Route Intelligence</h2>
        <p className="lede-tight">
          This mirrors the corridor widget we plan to surface on the assembly HUD so commanders inside
          Utopia can decide when to reroute fleets in real time.
        </p>
        {recomputeAction ? (
          ...
          <ul className="sector-list">
            {routes.map((route) => (
              <li key={`${route.origin}-${route.destination}`}>
                ...
                {route.topSignals.length ? (
                  <p className="status">Signals: {route.topSignals.join(", ")}</p>
                ) : null}
              </li>
            ))}
          </ul>
